...
-- Migration 001: Create profiles and couple relationships
-- =====================================================
...
-- Update events table to support couples
ALTER TABLE events ADD COLUMN IF NOT EXISTS couple_id UUID REFERENCES couple_relationships(id) ON DELETE CASCADE;
...
-- Update todos table to support couples and assignments
ALTER TABLE todos ADD COLUMN IF NOT EXISTS couple_id UUID REFERENCES couple_relationships(id) ON DELETE CASCADE;
ALTER TABLE todos ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id);
ALTER TABLE todos ADD COLUMN IF NOT EXISTS assigned_to UUID REFERENCES auth.users(id);
ALTER TABLE todos ADD COLUMN IF NOT EXISTS priority TEXT DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high'));
...
ALTER TABLE todos ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
